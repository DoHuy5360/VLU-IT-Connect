    ...
    <div class="container py-3">
        <div class="bg-white px-4 rounded rounded-lg border border-new-pale-gray">
            ...
            <div class="row gy-2 align-items-stretch">
                            ...
                            <div class="featured-article-box d-flex flex-column gap-3" style="cursor: pointer">
                                ...
                                <div>
                                    <div class="hover_underline">
                                        <h4 class="mb-sm-3 mb-2 clickable-text">{{ featuredArticle.title }}</h4>
                                        <p class="text-muted mb-sm-3 mb-0">{{ store.truncateText(featuredArticle.details, 600) }}</p>
                                    </div>
                ...
                <div v-else style="height: 100vh; display: grid; place-items: center">{{ store.isVietNamese() ? "Bài viết không tồn tại" : "No content" }}</div>
                <!-- Old Articles with Pagination -->
                <div class="col-lg-6 d-flex flex-column">
                    <div v-if="data.paginated.length" class="rounded d-flex flex-column py-sm-4 flex-grow-1">
                        <ul class="list-unstyled flex-grow-1">
                            <li v-for="(article, index) in data.paginated" :key="index" class="mb-4 rounded" style="cursor: pointer">
                                <RouterLink :to="`/blog/detail/${article.slug}`" class="text-black hover_underline">
            ...
            <div class="py-2 mb-3">
                <div class="d-flex gap-2 justify-content-between">
                    ...
                    <RouterLink :to="`/videos?category=${category}`" class="text-primary hover_underline" style="white-space: nowrap">
                ...
                <div v-if="videos.length !== 0" class="row" id="wrapVideo">
                    <div v-for="video in videos" class="col-auto col-sm-3">
                        <video v-if="store.isMP4(video.video)" :src="video.video" controls class="rounded w-100 h-50"></video>
                        <iframe v-else width="100%" height="200px" :src="video.video" frameborder="0" allowfullscreen class="rounded h-50" title="Guiding clips"></iframe>
                        <RouterLink :to="`/blog/detail/${video.slug}`" class="hover_underline text-black">
                            <div class="mt-2">
                ...
                <div v-else>{{ store.isVietNamese() ? "Không có nội dung" : "No content available" }}</div>
            </div>
        </div>
    </div>
</template>
