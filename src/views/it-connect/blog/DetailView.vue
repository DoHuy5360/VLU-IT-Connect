        ...
        <div v-if="featuredArticle !== null" class="rounded p-4">
          <div class="">
            <div class="d-flex gap-3 text-muted mb-3">
              ...
              <div class="d-flex gap-2 align-items-center">
                ...
                <RouterLink
                  :to="`/blog?category=${featuredArticle?.category?.slug}`"
                  class="hover_underline text-primary clickable-text"
                  >{{ featuredArticle?.category?.name }}</RouterLink
                >
              </div>
            </div>
            ...
            <div
              class="mb-3"
              id="blogContent"
              v-html="featuredArticle?.details"
            ></div>
            <div
              v-if="featuredArticle?.video !== null"
              class=""
              style="height: 50vh"
            >
              <iframe
                :src="featuredArticle?.video"
                width="100%"
                height="100%"
                frameborder="0"
                allowfullscreen
            ...
            <br />
            <div class="box-author-and-btn-share">
              <strong> {{ featuredArticle?.author }}</strong>
              <div class="share-container">
                <router-link
                  to="/notification/subscribe"
                  class="btn-notification ms-2"
                >
                  <svg
                    width="16"
                    height="16"
                ...
                <div class="share-dropdown" v-show="showShareMenu">
                  ...
                  <div class="share-item" @click="copyShareLink">
                    <svg
                      width="20"
                      height="20"
                      viewBox="0 0 24 24"
                    ...
                    >
                      <path
                        d="M13.2929 8.29289C13.6834 7.90237 14.3166 7.90237 14.7071 8.29289L19.7071 13.2929C20.0976 13.6834 20.0976 14.3166 19.7071 14.7071L14.7071 19.7071C14.3166 20.0976 13.6834 20.0976 13.2929 19.7071C12.9024 19.3166 12.9024 18.6834 13.2929 18.2929L16.5858 15H8C7.44772 15 7 14.5523 7 14V10C7 9.44772 7.44772 9 8 9C8.55228 9 9 9.44772 9 10V13H16.5858L13.2929 9.70711C12.9024 9.31658 12.9024 8.68342 13.2929 8.29289Z"
                        fill="#6c757d"
                      />
                      <path
                        d="M5 2C3.34315 2 2 3.34315 2 5V19C2 20.6569 3.34315 22 5 22H6C6.55228 22 7 21.5523 7 21C7 20.4477 6.55228 20 6 20H5C4.44772 20 4 19.5523 4 19V5C4 4.44772 4.44772 4 5 4H6C6.55228 4 7 3.55228 7 3C7 2.44772 6.55228 2 6 2H5Z"
                        fill="#6c757d"
                      />
                    </svg>
                    Copy link
          ...
          <div class="mt-4">
            <CommentView
              ...
              :allowComment="featuredArticle.allowComment"
              :commentCensorship="featuredArticle.commentCensorship"
            />
          </div>
        </div>
        <div
          v-else
          class="h-100 d-grid align-items-center"
          ...
          <div>
            ...
            >
              <li
                ...
                :key="index"
                class="hover_underline mb-1"
                style="cursor: pointer; padding-right: 1rem"
              >
                <RouterLink
                  :to="`/blog?category=${category.Slug}`"
                  class="text-black d-flex justify-content-between"
                >
                  <span>
            ...
            <div v-if="relatedArticles.length" class="d-flex flex-column gap-2">
              <div v-for="(article, index) in relatedArticles" :key="index">
                <RouterLink
                  ...
                  class="hover_underline row py-2 text-black"
                ...
                >
                  <div class="col-4 d-flex">
                    <img
                      ...
                      @error="
                        () => {
                          article.image = store.getBrandAsset(
                            '/logo-khong-chu.png'
                          );
                        }
                      "
                  ...
                  <div class="col-8">
                    <h6
                      class="mb-1 clickable-text text-truncate"
                      :title="article.title"
                    >
...
const shareOnTwitter = () => {
  const url = `https://twitter.com/intent/tweet?url=${encodeURIComponent(
    shareUrl.value
  )}&text=${encodeURIComponent(featuredArticle.value.title)}`;
  window.open(url, "_blank", "width=600,height=400");
  showShareMenu.value = false;
};
...
const getRelatedArticles = async () => {
  try {
    const response = await guestRequest.get(
      `/posts/categories-with-posts?categorySlug=${categoryOfThisPost.slug}&limit=5`
    );
    let categoryAndPosts = response.data;

    for (
      let categoryIndex = 0;
      categoryIndex < categoryAndPosts.length;
      categoryIndex++
    ) {
...
onMounted(async () => {
  ...
  // Add click event listener to close share menu when clicking outside
