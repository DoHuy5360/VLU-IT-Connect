<template>
    <div class="rounded-2 shadow-sm border border-new-gray py-2 px-4 h-100">
      ...
      <div class="d-flex flex-column gap-2">
        <div
          v-for="(blog, index) in paddedBlogs"
          :key="index"
          class="d-flex gap-2"
        >
      ...
      <div v-if="blogs.length > 4" class="text-end mt-2">
        <button class="btn btn-link p-0" @click="navigateToBlogList">
          Show More
        </button>
      </div>
    </div>
  ...
  export default {
    ...
    computed: {
      paddedBlogs() {
        const sliceCount = 4;
        return this.blogs.slice(0, sliceCount);
      },
    },
    methods: {
      navigateToBlogList() {
        this.$router.push({ name: "BlogList", query: { category: this.name } });
      },
    },
  };
  </script>
