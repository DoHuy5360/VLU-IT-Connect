import { authRequest } from "./axiosConfig";
...
const accountGroupService = {
    ...
    async getGroupById(groupId) {
        try {
            const response = await authRequest.get(`/AccountGroup/${groupId}`);
            return {
                success: true,
                data: response.data,
            };
    ...
    async createGroup(groupData) {
        try {
            const response = await authRequest.post("/AccountGroup/add", {
    ...
    async editGroup(groupId, groupData) {
        try {
            const response = await authRequest.put("/AccountGroup/edit", {
                groupId,
    ...
    async deleteGroup(groupId) {
        try {
            const response = await authRequest.delete(`/AccountGroup/delete/${groupId}`);
            return {
                success: true,
                data: response.data,
            };
    ...
    async deleteMultipleGroups(groupIds) {
        ...
        try {
            const response = await authRequest.delete("/AccountGroup/deletes", {
                data: groupIds,
            });
    ...
    async searchByName(searchTerm, pageNumber = 1, pageSize = 20) {
        try {
            const response = await authRequest.get("/AccountGroup/search", {
                params: {
                    searchTerm,
                    pageNumber,
                    pageSize,
    ...
    async getAvailablePermissions() {
        try {
            const response = await authRequest.get("/AccountGroup/available-permissions");
            return {
                success: true,
                data: response.data,
            };
    ...
    async getGroupPermissions(groupId) {
        try {
            const response = await authRequest.get(`/AccountGroup/permissions/${groupId}`);
    ...
    async assignUsersToGroup(groupName, userIds) {
        try {
            const response = await authRequest.post("/AccountGroup/assign-users-to-group", {
                GroupName: groupName,
                UserIds: userIds,
            });
            return {
            ...
            return {
                success: false,
                message: error.response?.data?.message || "Không thể thêm người dùng vào nhóm",
            };
        }
    },
};
