    ...
    <div class="content">
        <BaseBlock title="Danh sách thể loại" class="shadow-sm rounded">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <label for="searchCategoryInput">Tìm kiếm thể loại</label>
                <div class="col-md-4">
                    <input id="searchCategoryInput" v-model="searchTerm" type="text" placeholder="Tìm kiếm ..." class="form-control" @input="onSearch" />
                </div>
            </div>
    ...
    methods: {
        async getCategories() {
            try {
                ...
                if (categoriesData.length > 0) {
                    this.categories = categoriesData.map((category) => this.processCategories(category));
        ...
        processCategories(category) {
            ...
            if (category.Children && category.Children.$values) {
                newCategory.children = category.Children.$values.map((child) => this.processCategories(child)).filter((child) => child !== null);
            }
