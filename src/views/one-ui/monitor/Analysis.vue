    ...
    <div class="content">
        <div class="row g-4 mb-3">
            <div class="col-sm-6 col-xxl-3">
                <CardData :value="amountOfBlog || 'chưa cập nhật'" label="Tổng số bài viết">
                    <i class="si si-note fs-3 text-primary"></i>
                </CardData>
            </div>
            <div class="col-sm-6 col-xxl-3">
                <CardData :value="amountOfview || 'chưa cập nhật'" label="Tổng số lượt truy cập">
                    <i class="si si-eye fs-3 text-primary"></i>
                </CardData>
            </div>
            <div class="col-sm-6 col-xxl-3">
                <CardData :value="amountOfcomment || 'chưa cập nhật'" label="Tổng số bình luận">
...
async function getPosts() {
    const response = await guestRequest.get(`/posts?PageNumber=1&PageSize=9999&IsAdmin=true`);
...
const getviews = async () => {
    try {
        const response = await guestRequest.get(`https://localhost:7017/api/admin/posts/view-statistics`, {
            headers: {
