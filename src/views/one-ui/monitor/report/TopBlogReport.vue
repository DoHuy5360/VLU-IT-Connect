...
async function getrepsost() {
  ...
  try {
    ...
    const response = await fetch(
      "https://localhost:7017/api/Report/combined-statistics",
      {
        method: "GET",
        headers: {
...
async function exportReport() {
  try {
    ...
    const response = await fetch(
      "https://localhost:7017/api/ExportExcel/combined-statistics",
      {
        method: "GET",
        headers: {
          Accept:
    ...
    const blob = await response.blob(); // Nhận file Excel dưới dạng Blob
    const url = window.URL.createObjectURL(blob); // Tạo URL từ Blob
    const a = document.createElement("a");
    a.href = url;
    a.download = "BaoCaoChiTiet.xlsx"; // Đặt tên file tải về
...
function search(input) {
  ...
  if (searchValue === "") {
    ...
    if (posts.value.length != 0) {
      searchingPosts.value = posts.value.filter((post) =>
        containsSubstring(searchValue, post.Title)
      );
    }
  }
}
